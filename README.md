...
Photo-to-mascot prompts are generated by `createMascotPrompt` in `src/data/catalog.ts`. The scene title is inserted into a shared identity-preserving art direction so all 80 recipes stay consistent as the catalog grows.
